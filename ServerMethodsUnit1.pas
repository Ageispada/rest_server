...
// Cuando recibe la confirmación del mesero, por medio de esta función se actuliza los datos de la mesa y la cuenta
// Recibe por parametros una lista de códigos de items, una de sus cantidades requeridas y una de descripciones asociadas
var
...
begin
  ...
  FDQuery1.SQL.Clear;
  // Localiza el código del registro que está guardado las actividades sobre la mesa abierta
  ...
  begin
    // Recibe los códigos , cantidades y descripciones de las listas
    codigo:= '';
    cantidad := '';
    descripcion := '';
    if (vcodigo[1] = '<')   then
    ...
    begin
      // Se consutla la informacíón del artículo
      FDQuery1.SQL.Clear;
      ...
      // Para control de operación se registra un historial de las comandas hechas por fecha en la tabla TB_NUM
      begin
      ...
      FDQuery1.Close;
      unlock (2);
      // Se inserta la comanda en la base de datos
  ...
  begin
    // Se realiza la sumatoria de los montos de las comandas
    FDQuery2.SQL.Clear;
    ...
    FDQuery2.Close;
    // Se actualiza el monto en la cuenta del cliente
    FDQuery2.SQL.Clear;
...
// Al cerrar la mesa se solicita la impresión de la factura
var
...
function TServerMethods1.Cargardatos(usuario, clave, mesa, identificacion,
  ...
  // Actualiza los datos del cliente en una cuenta abierta
var
  Respuesta : String;
begin
  if Login(usuario, clave) then
...
//Esta función veifica si la mesa está abierta al insertar una nueva comanda, si no es así la abre en la base de datos
var
  codigo : integer;
begin
  codigo := 0;
...
function TServerMethods1.Listarclientes(usuario, clave, rif,
  ...
  // Función para listar los clientes registrado por uno o varios parámetros de búqueda
var
  Respuesta :  String;
...
// Devuelve al cliente la lista de productos pedidos en una mesa
var
  Respuesta :  String;
begin
  if Login(usuario, clave) then
...
// Devuelve los datos del cliente asocuiado a una mesa
var
...
// Devuelve una lista de items buscado por grupo o un parámetro de búsqueda
var
  Respuesta :  String;
  pvp : String ;
begin
...
// La función que es usada para listar las mesas y sus estados
var
  Respuesta :  String;
begin
  if Login(usuario, clave) then
...
// valida que las credenciales de usuario sena válidas para habilitar el acceso al sevidor
begin
  FDQuery1.SQL.Clear;
...
// Métodos de semáforos para bloque selectivo temporal de la base de datos
